...
def test_initial_file_organizer(file_organizer: FileOrganizer, folder_test_path: str) -> None:
...
def test_get_files_without_filters_and_extensions_allowed(file_organizer: FileOrganizer) -> None:
    files = file_organizer._get_files()

    assert files == ["test.py", "test.txt"]

def test_get_files_with_extensions_allowed_and_without_filters(file_organizer: FileOrganizer) -> None:
...
def test_get_files_with_filters_and_without_extensions_allowed(file_organizer: FileOrganizer) -> None:
    file_organizer.filters = {
        "min_size": 0,
        "max_size": 15
    }
...
def test_get_files_with_extensions_allowed_filters(file_organizer: FileOrganizer) -> None:
    file_organizer.filters = {
        "min_size": 0,
        "max_size": 15
    }
...
def test_organizer_not_path_or_extensions_allowed(file_organizer: FileOrganizer) -> None:
    file_organizer.all_path_extensions = []

    message, status = file_organizer.organizer()

    assert not status
    assert message == "It could not be organized because there is no path or there are no extensions."
...
def test_organizer_all_files(file_organizer: FileOrganizer, folder_test_path: str) -> None:
    file_organizer.extensions_allowed = []
    file_organizer._reset_state()
    ...
    folder_names = [name for name in os.listdir(folder_test_path) if os.path.isdir(f"{folder_test_path}/{name}")]
...
def test_organizer_with_extensions_allowed(file_organizer: FileOrganizer, folder_test_path: str) -> None:
    file_organizer.extensions_allowed = ["txt"]
    file_organizer._reset_state()
    ...
    assert f"TXT_{file_organizer.folder_name}" in folder_names
    assert not f"PY_{file_organizer.folder_name}" in folder_names
